...
if (!existsSync(`./${client.globalConfig}`))
	return logger('Không tìm thấy file config của bot!', 2);
...
if (
	existsSync(resolve('./includes', 'skeleton_data.sqlite')) &&
	!existsSync(resolve('./includes', 'data.sqlite'))
)
	copySync(
		resolve('./includes', 'skeleton_data.sqlite'),
		resolve('./includes', 'data.sqlite')
	);

axios
	.get(
		'https://raw.githubusercontent.com/catalizcs/miraiv2/master/package.json'
	)
	.then(res => {
		...
		if (semver.lt(local, res.data.version))
			logger(
				`Đã có phiên bản ${res.data.version} để bạn có thể cập nhật!`,
				'[ CHECK UPDATE ]'
			);
	...
	.catch(err =>
		logger(
			'Đã có lỗi xảy ra khi đang kiểm tra cập nhật cho bạn!',
			'[ CHECK UPDATE ]'
		)
	);
...
const commandFiles = readdirSync(join(__dirname, '/modules/commands')).filter(
	...
	try {
		...
		if (command.config.dependencies) {
			try {
				...
				logger(
					`Không tìm thấy gói phụ trợ cho module ${
						command.config.name
					}, tiến hành cài đặt: ${command.config.dependencies.join(', ')}!`,
					'[ LOADER ]'
				);
				execSync('npm install -s ' + command.config.dependencies.join(' '));
				delete require.cache[require.resolve(`./modules/commands/${file}`)];
				logger(
					`Đã cài đặt thành công toàn bộ gói phụ trợ cho module ${
						command.config.name
					}`,
					'[ LOADER ]'
				);
		...
		if (command.config.envConfig) {
			try {
				for (const [key, value] of Object.entries(command.config.envConfig)) {
					if (typeof __GLOBAL[command.config.name] == 'undefined')
						__GLOBAL[command.config.name] = new Object();
					if (typeof config[command.config.name] == 'undefined')
						config[command.config.name] = new Object();
					if (typeof config[command.config.name][key] !== 'undefined')
						__GLOBAL[command.config.name][key] =
							config[command.config.name][key];
					else __GLOBAL[command.config.name][key] = value || '';
					if (typeof config[command.config.name][key] == 'undefined')
						config[command.config.name][key] = value || '';
				}
				logger(`Loaded config module ${command.config.name}`, '[ LOADER ]');
			} catch (error) {
				console.log(error);
				logger(
					`Không thể tải config module ${command.config.name}`,
					'[ LOADER ]'
				);
			}
		}
		...
		logger(
			`Không thể load module command ${file} với lỗi: ${error.message}`,
			'[ LOADER ]'
		);
	}
}
...
const eventFiles = readdirSync(join(__dirname, '/modules/events')).filter(
...
for (const file of eventFiles) {
	...
	try {
		...
		if (event.config.envConfig) {
			try {
				for (const [key, value] of Object.entries(event.config.envConfig)) {
					if (typeof __GLOBAL[event.config.name] == 'undefined')
						__GLOBAL[event.config.name] = new Object();
					if (typeof config[event.config.name] == 'undefined')
						config[event.config.name] = new Object();
					if (typeof config[event.config.name][key] !== 'undefined')
						__GLOBAL[event.config.name][key] = config[event.config.name][key];
					else __GLOBAL[event.config.name][key] = value || '';
					if (typeof config[event.config.name][key] == 'undefined')
						config[event.config.name][key] = value || '';
				}
				logger(`Loaded config event module ${event.config.name}`, '[ LOADER ]');
			} catch (error) {
				logger(
					`Không thể tải config event module ${event.config.name}`,
					'[ LOADER ]'
				);
			}
		}
		...
		logger(
			`Không thể load module event ${file} với lỗi: ${error.message}`,
			'[ LOADER ]'
		);
	}
}
...
logger(
	`Load thành công: ${client.commands.size} module commands | ${
		client.events.size
	} module events`,
	'[ LOADER ]'
);
...
function onBot({ models }) {
	login({ appState: require(appStateFile) }, (err, api) => {
		...
		let listen = require('./includes/listen')({
			api,
			models,
			client,
			__GLOBAL
...
(async () => {
	...
	let completedMigrations = await sequelize.query(
